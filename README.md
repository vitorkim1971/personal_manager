...
### 테이블 구조
- **tasks**: 작업 정보
- **transactions**: 개인 거래 내역
- **projects**: 프로젝트 정보
- **budgets**: 예산 정보
- **files**: 파일 정보
- **company_accounts**: 회사 계좌 정보 (암호화폐 지갑 포함)
- **company_transactions**: 회사 거래 내역
- **settings**: 설정 정보

### 데이터베이스 재생성
스키마가 변경되었을 때 데이터베이스를 재생성하려면:

**Windows (PowerShell):**
```powershell
Remove-Item -Path "personal-manager.db" -Force
npm run dev
```

**macOS/Linux:**
```bash
rm personal-manager.db
npm run dev
```
